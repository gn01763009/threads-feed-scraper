...
> ⚠️ **2026-09 起，關鍵字搜尋 / 標籤 / 自訂 feed 三個模式暫不可用。**
> 原因不在這隻爬蟲：Threads 從那時起不再對「未登入」的訪客提供搜尋與標籤結果——那些資料它根本不送。
> 實測過（登出狀態的瀏覽器在搜尋頁拿到 0 篇貼文、0 個後端請求），任何不登入的方法都拿不到。
...
| 🏷️ ~~Hashtag~~ | *暫不可用 — Threads 不對未登入者提供* | — |
| 🔎 ~~Search~~ | *暫不可用 — 同上* | — |
| 📰 ~~Feed~~ | *暫不可用 — 同上* | — |
