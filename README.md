...
- **v0.3** — 新增用戶頁面抓取（`profileUrls`）；新增單篇貼文 + 留言抓取（`postUrls`）；新增媒體連結與類型擷取（`mediaUrls`、`mediaType`）；新增時間標準化（`publishedAtISO`）；新增搜尋排序（`searchSort`：熱門 / 最新）；新增日期範圍篩選（`dateFrom`、`dateTo`）；新增觀看數（`viewCount`）和引用數（`quoteCount`）
